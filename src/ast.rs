...
/// Some of them are used only in the opcodes file.
...
fn get_base_tokens(
    input: &str, 
    lines: &Vec<(usize, Vec<Range<usize>>)>
) -> Token {
    ...
    for (i, (number, line)) in lines.iter().enumerate() {
        ...
        for word in line {
            let word = &input[word.start..word.end];
            ...
            match c {
                ...
                '-' => push1!(MINUS, String::new()),
                '(' => push1!(AT0, String::new()),
                ')' => push1!(AT1, String::new()),
...
unsafe fn get_tree(root: Token) -> Token {
...
fn get_ast(input: &str, symbols: &Vec<String>) -> Token {
    // Get words from the source file.
    let lines = get_lines(input, symbols); 
    // Get the basic tokens.
    let root = get_base_tokens(input, &lines);
